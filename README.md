...
雀荘のユーザ管理、入退場管理、卓管理、成績入力、プレイヤー別成績閲覧をクラウドで管理するWebアプリです。

現在の本番導入ステップでは、まず手入力による成績管理を中心に運用します。カメラ連携、卓成績取得、自動採点は将来拡張として別画面・APIに残しています。
...
- ユーザ管理: ユーザID、名前、入退場状態、パスワード変更
- 卓管理: 入場中ユーザの卓割当、スタッフ席、卓の解散
- 成績入力: 通常成績または大会成績を選び、4人分の最終点数を入力して確定
- 成績照会: 通算成績、直近10半荘、大会成績を切り替え表示
- プレイヤー画面: ログインユーザ本人の成績のみ閲覧

## 店舗別ユーザ管理

ユーザは店舗ごとに管理されます。プレイヤーログインでは、店舗を分けるために以下の3項目を入力します。
...
ユーザIDは店舗内で一意です。同じユーザIDでも、店舗IDが違えば別ユーザとして扱えます。

## 将来拡張

以下は卓成績取得・自動採点へ進む段階で利用します。初期の成績管理導入では必須ではありません。
